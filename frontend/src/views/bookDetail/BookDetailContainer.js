...
function BookDetailContainer(bookInfoSeq) {
  ...
  function getBookInfo() {
    //책정보 데려오기
    axios
      .get(url + `/api/v1/bookinfos/${useParam.seq}`)
  ...
  function getBookReview() {
    //책 리뷰 가져오기
    axios
      .get(url + `/api/v1/bookreviews/bookinfos/${useParam.seq}`)
      .then(function (response) {
  ...
  function getBookLog() {
    //북로그 가져오기
    axios
      .get(url + `/api/v1/booklogs/bookinfos/${useParam.seq}`, {
        params: {
          size: 5,
          page: booklogPage,
        },
  ...
  function handleMyModalOpen() {
    if (jwtToken === "" || jwtToken === undefined || jwtToken === null) {
      ...
      userReview.starRating === null ||
      userReview.starRating === undefined ||
      userReview.starRating === 0
    ) {
  ...
  return (
    <BookDetailPresenter
      ...
      booklogs={booklogs}
      booklogPage={booklogPage}
      booklogPageHandler={booklogPageHandler}
      booklogOrderHandler={booklogOrderHandler}
      booklogTotalCnt={booklogTotalCnt}
      ...
      handleMyModalClose={handleMyModalClose}
      handleWriteModalClose={handleWriteModalClose}
      setMyModalOpen={setMyModalOpen}
      setWriteModalOpen={setWriteModalOpen}
      handleMyModalOpen={handleMyModalOpen}
