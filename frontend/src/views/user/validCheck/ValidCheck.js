import axios from 'axios'
    ...
    function checkId(email, url){  
        axios.get(url+`/api/v1/members/email/${email}/exist`)
    ...
    function checkNickname (nickname,url){
        ...
        for(let i=0; i<nickname.length;i++){
            if(escape(nickname.charAt(i)).length>4){ //한글일 경우
                koreanTotalByte+=2;
            }else{ //영어일 경우
                englishTotalByte+=1;
            }
        ...
        if(englishTotalByte===0 && (koreanTotalByte<4|| koreanTotalByte>16) ){
            ...
            return false;
        }else if(koreanTotalByte===0 && (englishTotalByte<3|| englishTotalByte>12) ){
            alert("영어닉네임의 경우 3글자 이상 12글자 이하여야 합니다. ");
            return false;
        }else{
            axios.get(url+`/api/v1/members/nickname/${nickname}/exist`)
            .then(function(response){
                if(response.data.status===200){
    ...
    function checkPassword(password, passwordConfirm,email,nickname,name){
        ...
        if(password!==passwordConfirm){ //비밀번호 비밀번호 확인
            alert("비번다름");
            return false;
        }
        if(!/^[a-zA-Z0-9!@#$%\^&*()]{8,12}$/.test(password)){ //숫자,영문자,특수문자 조합으로 8~12자리를 사용하는지 확인
    ...
    function checkNameLength(name){
       ...
       for(let i=0; i<name.length;i++){
        if(escape(name.charAt(i)).length>4){ //한글일 경우
            koreanTotalByte+=2;
        }else{ //영어일 경우
            englishTotalByte+=1;
        }
       ...
       else if(koreanTotalByte===0 && (englishTotalByte<3|| englishTotalByte>20) ){
   ...
   export {checkId, checkEmail, checkNickname, checkPassword, checkNameLength, checkPhoneNumber};